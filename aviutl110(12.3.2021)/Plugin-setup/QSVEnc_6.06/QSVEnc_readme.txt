...
QSVEncC は、上記のコマンドライン版で、おまけになります。
オプションの詳細については、同梱のQSVEncC_options.txtを御覧いただくか、
下記urlを参照ください。
...
【QSVEnc 使用方法 (簡易インストーラ使用)】
付属の簡易インストーラを使用する方法です。
手動で行う場合は、後述のQSVEnc 使用方法 (手動)を御覧ください。

1. ダウンロードしたQSVEnc_3.xx.zipを一度解凍します。
...
Visual Studio 2015, 2017 および 2019 の Visual C++ 再頒布可能パッケージ
https://aka.ms/vs/16/release/vc_redist.x86.exe
vc_redist.x86.exeを選択してください。

2. 
QSVEnc.auo、QSVEnc.ini、QSVEnc_stgフォルダ をAviutlのpluginフォルダにコピーします。

3. 
Aviutlを起動し、「その他」>「出力プラグイン情報」にQSVEncがあるか確かめます。
ここでQSVEncの表示がない場合、1.で必要なものを忘れている、あるいは失敗したなどが考えられます。
...
5.
QSVEncの設定画面を開き、各実行ファイルの場所を指定します。
あと適当に設定します。
...
【どうでもいいメモ】
2021.12.1 (6.06)
・スレッドの優先度とPower Throttolingモードを指定するオプションを追加。(--thread-priority, --thread-throttoling)
